...
public class ThreadManager {
	...
	/**
	 * General method that prints the string parameter on the
	 * standard output.
	 * 
	 * @param str
...
//ADDED_abaixo
	 ...
	 */
	private void updateThreadStateConsideringEndOfAssociation(
			Thread t, ThreadState state, 
			Object associatedObject) {
		if (state.equals(ThreadState.FINISHED)) {
			ThreadAssociation tAssociation = 
				this.threadsAssociationByThread
				.get(t).getThreadAssociationBetween(
				t, associatedObject);
			if (tAssociation.definesGlobalEndOfThread()) {
				ThreadState previousState = this.getThreadState(t);
				this.threadsByState.changeThreadState(
					t, previousState,
	...
	private void updateThreadStateConsideringNewAssociation(Thread t,
		...
		if (state.equals(ThreadState.RUNNING)) {
			List<ThreadAssociation> associations = 
				this.threadsAssociationByThread
				.get(t).getThreadAssociationsList();
			for (ThreadAssociation assoc : associations) {//TODO: REVER CîDIGO
				if (!assoc.getState().equals(state)
					&& !(assoc.getState().equals(ThreadState.FINISHED) //n‹o Ž o caso em que estado Ž finished e a thread Ž diferente do associated object
					&& !assoc.getObject().equals(assoc.getThread()))
					//&& !(assoc.getState().equals(ThreadState.STARTED) //TODO:REVER:n‹o Ž o caso em que estado Ž started e a thread Ž diferente do associated object
					//&& !assoc.getObject().equals(assoc.getThread()))
						) {
			...
			throw new RuntimeException(
					"Unexpected state transition to  state ["+state
					+"]  considering [:"
					+ associatedObject.getClass().getCanonicalName()
					+ "]");
		}
